...
public class ModelEvaluationService : IModelEvaluationService
{
        ...
        NfcbetsContext context)
    {
        ...
        foreach (var roundGroup in roundGroups)
        {
            ...
            foreach (var feature in roundGroup.Take(3)) // Sample 3 pirates per round
            {
                ...
                if (futureDataCount > 0)
                {
                    ...
                    leakageIssues.Add(
                        $"Round {roundId}, Pirate {feature.PirateId}: Historical features may include future rounds: {string.Join(",", futureRounds.Take(3))}");
                }
            }
        }
        ...
        report.HasLeakage = leakageIssues.Any(issue =>
            issue.Contains("CRITICAL") || issue.Contains("exceeds") || issue.Contains("future"));

        if (report.HasLeakage)
        {
            Console.WriteLine(
                $"❌ Found {leakageIssues.Count(issue => issue.Contains("CRITICAL") || issue.Contains("exceeds"))} critical issues:");
            foreach (var issue in leakageIssues.Where(i =>
                         i.Contains("CRITICAL") || i.Contains("exceeds") || i.Contains("future")))
        ...
        else if (leakageIssues.Any())
        {
            ...
            foreach (var issue in leakageIssues.Take(3)) Console.WriteLine($"   {issue}");
