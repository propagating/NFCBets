...
namespace NFCBets.Services
{
    public class ModelEvaluationService : IModelEvaluationService
    {
        ...
        public async Task<FeatureImportanceReport> AnalyzeFeatureImportanceAsync(List<PirateFeatureRecord> trainingData)
        {
         ...
         public async Task<DataLeakageReport> CheckForDataLeakageAsync(List<PirateFeatureRecord> features, NfcbetsContext context)
    {
        ...
        foreach (var feature in features.Where(f => f.IsWinner.HasValue).Take(50)) // Sample check
        {
            if (feature.IsWinner.Value)
            {
                ...
                {
                    leakageIssues.Add($"Round {feature.RoundId}, Pirate {feature.PirateId}: Suspiciously high historical win rate ({feature.HistoricalWinRate:P2})");
                }

                if (feature.WinRateVsCurrentRivals > 0.95)
                {
                    leakageIssues.Add($"Round {feature.RoundId}, Pirate {feature.PirateId}: Suspiciously high rival win rate ({feature.WinRateVsCurrentRivals:P2})");
                }
            }
        }
        ...
        foreach (var roundGroup in roundGroups.Take(10))
        {
            ...
            foreach (var feature in roundGroup.Take(5)) // Sample 5 pirates per round
            {
                ...
                {
                    leakageIssues.Add($"Round {roundId}, Pirate {feature.PirateId}: TotalAppearances={feature.TotalAppearances} exceeds historical={actualHistoricalCount}");
                }
            }
        }
        ...
        if (sortedFeatures.Count >= 100)
        {
            ...
            var trainRounds = sortedFeatures.Take(splitPoint).Select(f => f.RoundId).Distinct().OrderBy(r => r).ToList();
            ...
            if (overlap.Any())
            {
                leakageIssues.Add($"Train/test overlap: {overlap.Count} rounds appear in both sets");
            }

            // Ensure test rounds come AFTER train rounds
            if (trainRounds.Any() && testRounds.Any() && trainRounds.Max() >= testRounds.Min())
            {
                leakageIssues.Add($"Temporal violation: Train rounds ({trainRounds.Max()}) overlap with test rounds ({testRounds.Min()})");
            }
        ...
        if (report.HasLeakage)
        {
            ...
            foreach (var issue in leakageIssues.Take(5))
            {
        ...
        private async Task<double> TrainAndGetAUC(List<MlPirateFeature> data, string? excludedFeature)
        {
            ...
            {
                featureColumns.Remove(excludedFeature);
            }
        ...
        private Dictionary<string, PerformanceMetrics> AnalyzeByOddsRange(List<MlPirateFeature> actual, List<PiratePredictionOutput> predictions)
        {
        ...
        private Dictionary<int, PerformanceMetrics> AnalyzeByFoodAdjustment(List<MlPirateFeature> actual, List<PiratePredictionOutput> predictions)
        {
            var results = new Dictionary<int, PerformanceMetrics>();

            for (int adj = -3; adj <= 3; adj++)
            {
                var indices = actual
                    .Select((a, i) => new { Actual = a, Index = i })
                ...
                results[adj] = new PerformanceMetrics
                {
                    ...
                    Calibration = Math.Abs(predictionsInRange.Average() - (actualInRange.Count(w => w) / (double)actualInRange.Count))
                };
            }
        ...
        private CalibrationMetrics AnalyzeCalibration(List<MlPirateFeature> actual, List<PiratePredictionOutput> predictions)
        {
            var bins = 10;
        ...
        private void DisplayEvaluationReport(ModelEvaluationReport report)
        {
            ...
            {
                Console.WriteLine($"   {range,-12}: Acc={metrics.Accuracy:P2}, Pred={metrics.AveragePredictedProbability:P2}, Actual={metrics.ActualWinRate:P2}, Cal={metrics.Calibration:F4}");
            }

            Console.WriteLine($"\n🍕 Performance by Food Adjustment:");
            foreach (var (adj, metrics) in report.PerformanceByFoodAdjustment.OrderBy(kv => kv.Key))
            {
                Console.WriteLine($"   {adj,3}: Acc={metrics.Accuracy:P2}, Pred={metrics.AveragePredictedProbability:P2}, Actual={metrics.ActualWinRate:P2}, n={metrics.Count}");
            }
            ...
            {
                Console.WriteLine($"   {bin.ProbabilityRange,-10}: Pred={bin.AveragePredictedProbability:P2}, Actual={bin.ActualWinRate:P2}, Error={bin.Calibration:F4}, n={bin.Count}");
            }
