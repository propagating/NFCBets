...
public async Task<List<int>> CollectRangeAsync(int startRound, int endRound, bool forceCollect = false, int maxParallel = 5)
{
    ...
    int maxParallel = 5)
{
    ...
    {
        Console.WriteLine($"   Re-collected: {completedRounds.Intersect(successfulRounds).Count()} existing rounds");
    }
    ...
    try
    {
        ...
        for (var arenaIndex = 0; arenaIndex < roundData.Pirates.Count; arenaIndex++)
        {
            ...
            for (var position = 0; position < pirateIds.Count; position++)
            {
                ...
                {
                    Console.WriteLine($"   ⚠️ Warning: Found 1:1 odds at arena {arenaId}, position {position} (oddsPosition {oddsPosition}) for pirate {pirateId}");
                    continue;
                }
            ...
            if (roundData.Winners?.Any() == true && roundData.Winners?.Any(x=> x == 0)== false)
            {
                ...
                for (var position = 0; position < pirateIds.Count; position++)
                {
                    ...
                    {
                        Console.WriteLine($"   ⚠️ Warning: Found 1:1 ending odds at arena {arenaId} for pirate {pirateId}");
                        continue;
                    }
...
private void ValidateRoundDataStructure(FoodClubRoundData roundData)
{
    ...
    for (var arenaIndex = 0; arenaIndex < roundData.Pirates.Count; arenaIndex++)
    {
        ...
        {
            Console.WriteLine($"   ⚠️ Arena {arenaId} (API index {arenaIndex}): {pirateCount} pirates but {openingOddsCount} opening odds (expected {expectedOddsCount})");
        }
        
        if (currentOddsCount != expectedOddsCount)
        {
            Console.WriteLine($"   ⚠️ Arena {arenaId} (API index {arenaIndex}): {pirateCount} pirates but {currentOddsCount} current odds (expected {expectedOddsCount})");
        }
        
        // Validate placeholder at position 0
        if (roundData.OpeningOdds[arenaIndex][0] != 1)
        {
            Console.WriteLine($"   ⚠️ Arena {arenaId}: opening odds[0] = {roundData.OpeningOdds[arenaIndex][0]}, expected 1 (placeholder)");
        }
        
        if (roundData.CurrentOdds[arenaIndex][0] != 1)
        {
            Console.WriteLine($"   ⚠️ Arena {arenaId}: current odds[0] = {roundData.CurrentOdds[arenaIndex][0]}, expected 1 (placeholder)");
        }
        ...
        for (var position = 0; position < pirateCount; position++)
        {
            ...
            {
                Console.WriteLine($"   ⚠️ Arena {arenaId}, pirate position {position}: opening odds = 1 (unexpected - should be ≥2)");
            }
            
            if (roundData.CurrentOdds[arenaIndex][oddsPosition] == 1)
            {
                Console.WriteLine($"   ⚠️ Arena {arenaId}, pirate position {position}: current odds = 1 (unexpected - should be ≥2)");
            }
        }
    }
    ...
    if (roundData.Winners?.Any() == true)
    {
        ...
        {
            Console.WriteLine($"   ⚠️ Round {roundData.Round}: Winners array contains all zeros, indicating the round is not yet complete. Skipping winner validation.");
            return;
        }
        for (var arenaIndex = 0; arenaIndex < roundData.Winners.Count; arenaIndex++)
        {
            ...
            {
                Console.WriteLine($"   ⚠️ Arena {arenaId}: winner position = {winnerPosition}, expected 1-{pirateCount}");
            }
            ...
            {
                Console.WriteLine($"   ⚠️ Arena {arenaId}: winner position {winnerPosition} converts to invalid pirate index {pirateIndex}");
            }
        }
    }
